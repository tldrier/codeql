...
import com.semmle.extractor.html.HtmlExtractor;
...
public class JavaScriptHTMLExtractor implements IExtractor {

	private class JavaScriptHTMLElementHandler implements HtmlExtractor.ElementHandler {
  ...
  public JavaScriptHTMLExtractor(ExtractorConfig config, ExtractorState state) {
  ...
  public LoCInfo extract(TextualExtractor textualExtractor) {
	...
	HtmlExtractor extractor = new HtmlExtractor(textualExtractor.getSource(), textualExtractor.getTrapwriter(), textualExtractor.getLocationManager().getFileLabel());
	
	extractor.extract(eltHandler);
